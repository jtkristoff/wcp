...
and routers are still relatively new technology.  Some of the base
specifications were only finalized and published in 2012.  Deployment
began in earnest and by 2013 public monitoring emerged to track the
technology's deployment status and to collect
statistics.\cite{nist_rpki_2019}  One of the earliest research papers
focused on identifying how the RPKI was being used, paying particular
attention to the quality of ROAs network operators would putting into
the RPKI.\cite{iamartino_measuring_2015}  This work first broached the
subject of evaluating ROV by performing an \emph{offline} analysis,
comparing a snapshot of published ROAs with a snapshot of routing data
from the Route Views project.\cite{oregon_routeviews_2019}  This work
however did not attempt to discover if ROV was actively being performed
on the Internet.  Approaches to measuring deployment of ROV is covered
in the remainder of this section.

\subsection{Uncontrolled Passive Measurement}\label{subsec:Uncontrolled Passive Measurement}

RPKI repository data, and specifically the ROAs, can be freely and
easily retrieved.  Normally the ROAs are ultimately used by routers to
perform ROV, but can be useful for other purposes such as RPKI usage
analysis and measurement projects.  Coupled with route collector systems
such as those provided by RouteViews, researchers can perform a number
of studies based on real-world data without having direct access to
Internet routers.  In the literature this is referred to as uncontrolled
passive measurement.
...
policy decision, unpublished and generally unknowable to those
unaffiliated with the local network administration.  The authors in
\cite{gilad_are_2017} propose a method to uncover ROV usage in the
Internet by evaluating when an AS originates both valid and invalid
route announcements.   By examining different RouteViews vantage points,
termed sensors by the authors, their technique infers ROV policy on the
interceding path when one or both types (valid or invalid) of routes
arrive at a sensor node.  The supposition is that if both routes arrive
at a sensor, ROV is not in use by any AS listed in the AS path
attribute.  If only the valid route arrives at a sensor node, the
authors consider this evidence of ROV somewhere along the AS path.
...
prefixes, one invalid, another valid according to the RPKI.  If AS 64501
is performing ROV, only the valid route will be seen by sensor 1.
Conversely, if AS 64502 does not perform ROV, both the valid and invalid
routes will be seen by sensor 2.
...
perform ROV may receive and process competing invalid and valid routes,
...
approach is that it requires the origination of an invalid route.  This
may be a relatively rare event.
...
could not duplicate these results.  The response paper found
discprecpancies in results when they varied the number of sensors used,
which may partially explain the different outcomes in the two papers.

Publishing ROAs, at least for address space that is not sub-allocated to
other customers, is a relatively simple operation compared to ROV.  It
would stand to reason that a network performing ROV is likely to also be
publishing ROAs, but the converse seems unlikely.  Neither paper
explored this correlation however.

\subsection{Active Controlled Measurement}
